...
# === NEW TEXT DISPLAY ADDED HERE ===
train_zone_ax = plt.axes([0.77, 0.57, 0.2, 0.045])
train_zone_text = train_zone_ax.text(0.5, 0.5, '', ha='center', va='center', fontsize=10)
train_zone_ax.set_axis_off()
...
def train_and_plot_nn(event=None):
    ...
    # # Sort the training data to check for signature changes
    # sorted_indices = np.argsort(x_train.flatten())
    # sorted_x_train = x_train[sorted_indices]
    
    # # Calculate signatures for the sorted training data
    # current_output = tf.convert_to_tensor(sorted_x_train, dtype=tf.float32)
    # binary_activations_train = []
    # for layer in nnet.nnet.layers:
    #     current_output = layer(current_output)
    #     if hasattr(layer, 'activation') and layer.activation.__name__ == 'relu':
    #         binary_activations_train.append(tf.cast(current_output > 0, tf.int8))
    
    # if binary_activations_train:
    #     train_signatures = tf.concat([tf.reshape(act, (act.shape[0], -1)) for act in binary_activations_train], axis=1).numpy()
    #     # Count how many times the signature changes between consecutive training points
    #     num_train_changes = np.sum(np.any(np.diff(train_signatures, axis=0), axis=1))
    #     num_train_zones = num_train_changes + 1
    #     train_zone_text.set_text(f'Zones train: {num_train_zones}')
    # else: # Handle case with no ReLU layers
    #     train_zone_text.set_text('Zones train: 1')
    # # === END OF NEW CALCULATION ===

    if activation_lines_visible:
        toggle_activation_lines()
        toggle_activation_lines()
...
def toggle_activation_lines(event=None):
    ...
    if not activation_lines_visible:
        if nnet is None:
            print("Veuillez d'abord entraîner un réseau.")
            return
        # Computation for a grid of x values
        ...
        # Computation for the training points
        offsets = sample_scatter.get_offsets()
        x_train = offsets[:, 0].reshape(-1, 1)
        sorted_indices = np.argsort(x_train.flatten())
        sorted_x_train = x_train[sorted_indices]
        change_points = nnet.get_activation_change_points(sorted_x_train)
        for x in change_points:
            line = input_ax.axvline(x=x, color='black', linestyle='dotted', linewidth=1)
            activation_lines.append(line)
        num_train_zones = len(change_points) + 1
        train_zone_text.set_text(f'Zones X_train: {num_train_zones}')
        toggle_activation_button.label.set_text('Activation cachée')
    else:
        affine_zone_text.set_text('')
        train_zone_text.set_text('') # Also clear the training zones text
